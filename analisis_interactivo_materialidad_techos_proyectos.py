...
# =========================================================
# VARIABLES REALES DE MATERIALIDAD DE TECHOS
# =========================================================
# Denominador: n_vp_ocupada = viviendas particulares ocupadas
#
# Estas columnas son las que aparecieron en tu base:
# n_mat_techo_tejas
...
VARIABLES_TECHOS = {
    "n_mat_techo_zinc": "Planchas metálicas / zinc",
    "n_mat_techo_hormigon": "Losa de hormigón",
    "n_mat_techo_fibrocemento": "Planchas de fibrocemento",
    "n_mat_techo_tejas": "Tejas o tejuelas",
    "n_mat_techo_fonolita": "Fonolita",
    "n_mat_techo_paja": "Paja u otros vegetales",
    "n_mat_techo_precarios": "Materiales precarios",
    "n_mat_techo_sin_cubierta": "Sin cubierta sólida"
}

VARIABLES_NECESARIAS = list(VARIABLES_TECHOS.keys()) + ["n_vp_ocupada"]
...
for col in VARIABLES_NECESARIAS:
    if col not in df.columns:
        print("\nColumnas de techo encontradas en la base:")
        for c in df.columns:
            if "techo" in c.lower():
                print(" -", c)
...
print("=" * 90)
print("ANÁLISIS INTERACTIVO MATERIALIDAD DE TECHOS - PROYECTOS")
print("=" * 90)
print("\nBase completa:", df.shape)
print("Región Metropolitana:", region_rm.shape)

print("\nColumnas utilizadas:")
for col, label in VARIABLES_TECHOS.items():
    print(f"{label}: {col}")
...
orden_variables = list(VARIABLES_TECHOS.keys())
orden_labels = list(VARIABLES_TECHOS.values())
...
for nombre_proyecto, manzent in PROYECTOS.items():
    ...
    for i, categoria in enumerate(orden_labels):
        ...
        registros_brechas_excel.append({
            ...
            "Comuna proyecto": comuna_nombre,
            "MANZENT": manzent,
            "Materialidad de techos": categoria,
...
ruta_excel = SALIDAS_DIR / "28_resumen_materialidad_techos_proyectos.xlsx"
...
with pd.ExcelWriter(ruta_excel, engine="openpyxl") as writer:
    ...
    df_excel.to_excel(writer, sheet_name="Datos_techos", index=False)
    df_brechas_excel.to_excel(writer, sheet_name="Brechas_techos", index=False)
...
ruta_html = SALIDAS_DIR / "29_interactivo_materialidad_techos_proyectos.html"
...
<head>
    <meta charset="UTF-8">
    <title>Materialidad de techos en viviendas particulares ocupadas</title>
...
<body>
    <div class="container">
        <main class="main">
            <div class="header">
                <h1>Materialidad de techos en viviendas particulares ocupadas</h1>
                <p id="subtitle"></p>
            </div>
        ...
        <aside class="sidebar">
            <div class="sidebar-card">
                ...
                <div class="note">
                    La página 2 muestra la distribución porcentual de la materialidad predominante de techos sobre el total de viviendas particulares ocupadas de cada nivel territorial.
                </div>
            </div>
        </aside>
    </div>
    ...
    <script>
        ...
        function buildLayout() {{
            ...
            if (isConteos) {{
                pageTitleText = "Página 1: Viviendas por materialidad predominante de techos";
                pageSubtitleText = "Comparación de magnitudes absolutas entre Región Metropolitana, comuna y manzana.";
            }}

            if (isPorcentajes) {{
                pageTitleText = "Página 2: Distribución porcentual de la materialidad de techos";
                pageSubtitleText = "Comparación proporcional sobre viviendas particulares ocupadas.";
            }}

            if (isBrechas) {{
                pageTitleText = "Página 3: Matriz de color de brechas por materialidad de techos";
            ...
            if (isConteos && currentScale === "log") {{
                yAxisConfig.range = [0, LOG_UPPER_BOUND];
            }}
