...
VARIABLES_CIUO = {
    "n_ciuo_9": "Ocupaciones elementales",
    "n_ciuo_5": "Servicios y vendedores",
    "n_ciuo_7": "Artesanos y operarios",
    "n_ciuo_8": "Operadores de máquinas",
    "n_ciuo_4": "Apoyo administrativo",
    "n_ciuo_3": "Técnicos nivel medio",
    "n_ciuo_2": "Profesionales",
    "n_ciuo_6": "Agricultores y pesqueros",
    "n_ciuo_1": "Directores y gerentes",
    "n_ciuo_0": "Fuerzas armadas"
}

VARIABLES_NECESARIAS = list(VARIABLES_CIUO.keys()) + ["n_ocupado"]
...
print("=" * 90)
print("ANÁLISIS INTERACTIVO CIUO - PROYECTOS")
print("=" * 90)
...
for nombre_proyecto, manzent in PROYECTOS.items():
    ...
    # Ordenamos según el peso de la manzana del proyecto
    suma_manzana = manzana[VARIABLES_NECESARIAS].sum()

    variables_ordenadas = []
    for var, nombre in VARIABLES_CIUO.items():
        valor_manzana = float(suma_manzana[var])
        if valor_manzana > 0:
            variables_ordenadas.append((var, nombre, valor_manzana))
    ...
    # Si la manzana no tiene datos, dejamos todas las categorías
    ...
    orden_variables = [x[0] for x in variables_ordenadas]
    orden_labels = [x[1] for x in variables_ordenadas]
    ...
    for nivel_key, base in bases.items():
        ...
        total_ocupados = suma["n_ocupado"]

        valores_conteos = [suma[var] for var in orden_variables]
        valores_porcentajes = [
            safe_pct(suma[var], total_ocupados)
        ...
        for var, label, valor_conteo, valor_pct in zip(
            ...
            registros_excel.append({
                ...
                "MANZENT": manzent,
                "Variable": var,
                "Grupo ocupacional": label,
                "Nivel": nivel_key,
                "Personas ocupadas": float(valor_conteo),
                "Total ocupados nivel": float(total_ocupados),
                "% sobre ocupados": float(valor_pct)
            })
    ...
    for i, grupo in enumerate(orden_labels):
...
ruta_excel = SALIDAS_DIR / "16_resumen_ciuo_proyectos.xlsx"

with pd.ExcelWriter(ruta_excel, engine="openpyxl") as writer:
    df_detalle.to_excel(writer, sheet_name="Detalle_proyectos", index=False)
    df_excel.to_excel(writer, sheet_name="Datos_CIUO", index=False)
    df_brechas_excel.to_excel(writer, sheet_name="Brechas_CIUO", index=False)
...
ruta_html = SALIDAS_DIR / "17_interactivo_ciuo_proyectos.html"
...
<head>
    <meta charset="UTF-8">
    <title>Comparación territorial de grupos ocupacionales CIUO</title>
...
<body>
    <div class="container">
        <main class="main">
            <div class="header">
                <h1>Comparación territorial de grupos ocupacionales CIUO</h1>
                <p id="subtitle"></p>
            </div>
        ...
        <aside class="sidebar">
            <div class="sidebar-card">
                ...
                <div class="control-block">
                    ...
                    <div class="tab-buttons">
                        <button class="tab-button active" id="btnConteos">
                            Página 1 — Personas ocupadas
                        </button>
                        <button class="tab-button" id="btnPorcentajes">
                            Página 2 — % sobre ocupados
                        </button>
                ...
                <div class="note">
                    La página 3 muestra brechas en puntos porcentuales. Valores positivos indican que la manzana tiene mayor concentración relativa que el nivel comparado; valores negativos indican menor concentración relativa.
                </div>
            </div>
        </aside>
    </div>
    ...
    <script>
        ...
        function buildLayout() {{
            ...
            if (isConteos) {{
                pageTitleText = "Página 1: Personas ocupadas según grupo ocupacional CIUO";
                pageSubtitleText = "Comparación de magnitudes absolutas entre Región Metropolitana, comuna y manzana.";
            }}

            if (isPorcentajes) {{
                pageTitleText = "Página 2: Composición porcentual según grupo ocupacional CIUO";
                pageSubtitleText = "Comparación proporcional sobre el total de personas ocupadas de cada nivel territorial.";
            }}

            if (isBrechas) {{
                pageTitleText = "Página 3: Matriz de color de brechas ocupacionales";
