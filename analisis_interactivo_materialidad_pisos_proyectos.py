...
# =========================================================
# VARIABLES REALES DE MATERIALIDAD DE PISOS
# =========================================================
# Denominador: n_vp_ocupada = viviendas particulares ocupadas

VARIABLES_PISOS = {
    "n_mat_piso_radier_con_revestimiento": "Piso revestido sobre radier o vigas",
    "n_mat_piso_radier_sin_revestimiento": "Radier sin revestimiento",
    "n_mat_piso_baldosa_cemento": "Baldosa de cemento",
    "n_mat_piso_capa_cemento": "Capa de cemento",
    "n_mat_piso_tierra": "Tierra"
}

VARIABLES_NECESARIAS = list(VARIABLES_PISOS.keys()) + ["n_vp_ocupada"]
...
for col in VARIABLES_NECESARIAS:
    if col not in df.columns:
        print("\nColumnas de piso encontradas en la base:")
        for c in df.columns:
            if "piso" in c.lower() or "radier" in c.lower() or "baldosa" in c.lower() or "tierra" in c.lower():
                print(" -", c)
...
print("=" * 90)
print("ANÁLISIS INTERACTIVO MATERIALIDAD DE PISOS - PROYECTOS")
print("=" * 90)
print("\nBase completa:", df.shape)
print("Región Metropolitana:", region_rm.shape)

print("\nColumnas utilizadas:")
for col, label in VARIABLES_PISOS.items():
    print(f"{label}: {col}")
...
orden_variables = list(VARIABLES_PISOS.keys())
orden_labels = list(VARIABLES_PISOS.values())
...
for nombre_proyecto, manzent in PROYECTOS.items():
    ...
    for i, categoria in enumerate(orden_labels):
        ...
        registros_brechas_excel.append({
            ...
            "Comuna proyecto": comuna_nombre,
            "MANZENT": manzent,
            "Materialidad de pisos": categoria,
...
ruta_excel = SALIDAS_DIR / "30_resumen_materialidad_pisos_proyectos.xlsx"
...
with pd.ExcelWriter(ruta_excel, engine="openpyxl") as writer:
    ...
    df_excel.to_excel(writer, sheet_name="Datos_pisos", index=False)
    df_brechas_excel.to_excel(writer, sheet_name="Brechas_pisos", index=False)
...
ruta_html = SALIDAS_DIR / "31_interactivo_materialidad_pisos_proyectos.html"
...
<head>
    <meta charset="UTF-8">
    <title>Materialidad de pisos en viviendas particulares ocupadas</title>
...
<body>
    <div class="container">
        <main class="main">
            <div class="header">
                <h1>Materialidad de pisos en viviendas particulares ocupadas</h1>
                <p id="subtitle"></p>
            </div>
        ...
        <aside class="sidebar">
            <div class="sidebar-card">
                ...
                <div class="note">
                    La página 2 muestra la distribución porcentual de la materialidad predominante de pisos sobre el total de viviendas particulares ocupadas de cada nivel territorial.
                </div>
            </div>
        </aside>
    </div>
    ...
    <script>
        ...
        function buildLayout() {{
            ...
            if (isConteos) {{
                pageTitleText = "Página 1: Viviendas por materialidad predominante de pisos";
                pageSubtitleText = "Comparación de magnitudes absolutas entre Región Metropolitana, comuna y manzana.";
            }}

            if (isPorcentajes) {{
                pageTitleText = "Página 2: Distribución porcentual de la materialidad de pisos";
                pageSubtitleText = "Comparación proporcional sobre viviendas particulares ocupadas.";
            }}

            if (isBrechas) {{
                pageTitleText = "Página 3: Matriz de color de brechas por materialidad de pisos";
