...
# =========================================================
# VARIABLES DISCAPACIDAD Y DIFICULTADES
# =========================================================
# Denominador: n_per = población total

VARIABLES_DISCAPACIDAD = {
    "n_discapacidad": "Personas con discapacidad",
    "n_dificultad_ver": "Dificultad para ver",
    "n_dificultad_cogni": "Dificultad para recordar o concentrarse",
    "n_dificultad_mover": "Dificultad para caminar o subir escaleras",
    "n_dificultad_oir": "Dificultad para oír",
    "n_dificultad_comunic": "Dificultad para hablar o comunicarse",
    "n_dificultad_cuidado": "Dificultad para realizar tareas de cuidado personal"
}

VARIABLES_NECESARIAS = list(VARIABLES_DISCAPACIDAD.keys()) + ["n_per"]
...
print("=" * 90)
print("ANÁLISIS INTERACTIVO DISCAPACIDAD Y DIFICULTADES - PROYECTOS")
print("=" * 90)
...
orden_variables = list(VARIABLES_DISCAPACIDAD.keys())
orden_labels = list(VARIABLES_DISCAPACIDAD.values())
...
for nombre_proyecto, manzent in PROYECTOS.items():
    ...
    for nivel_key, base in bases.items():
        ...
        valores_porcentajes = [
            safe_pct(float(suma[var]), total_personas)
        ...
        for var, label, valor_conteo, valor_pct in zip(
            ...
            registros_excel.append({
                ...
                "MANZENT": manzent,
                "Variable": var,
                "Indicador discapacidad/dificultad": label,
                "Nivel": nivel_key,
                "Personas": float(valor_conteo),
                "Población total nivel": float(total_personas),
                "% sobre población total": float(valor_pct)
            })
    ...
    for i, categoria in enumerate(orden_labels):
        ...
        registros_brechas_excel.append({
            ...
            "Comuna proyecto": comuna_nombre,
            "MANZENT": manzent,
            "Indicador discapacidad/dificultad": categoria,
...
ruta_excel = SALIDAS_DIR / "24_resumen_discapacidad_proyectos.xlsx"

with pd.ExcelWriter(ruta_excel, engine="openpyxl") as writer:
    df_detalle.to_excel(writer, sheet_name="Detalle_proyectos", index=False)
    df_excel.to_excel(writer, sheet_name="Datos_discapacidad", index=False)
    df_brechas_excel.to_excel(writer, sheet_name="Brechas_discapacidad", index=False)
...
ruta_html = SALIDAS_DIR / "25_interactivo_discapacidad_proyectos.html"
...
<head>
    <meta charset="UTF-8">
    <title>Discapacidad y dificultades: comparación territorial</title>
...
<body>
    <div class="container">
        <main class="main">
            <div class="header">
                <h1>Discapacidad y dificultades: comparación territorial</h1>
                <p id="subtitle"></p>
            </div>
        ...
        <aside class="sidebar">
            <div class="sidebar-card">
                ...
                <div class="control-block">
                    ...
                    <div class="tab-buttons">
                        <button class="tab-button active" id="btnConteos">
                            Página 1 — Personas por indicador
                        </button>
                        <button class="tab-button" id="btnPorcentajes">
                            Página 2 — % sobre población total
                        </button>
                ...
                <div class="note">
                    Las dificultades pueden superponerse: una misma persona puede declarar más de una dificultad. Por eso los porcentajes no deben sumarse entre categorías.
                </div>
            </div>
        </aside>
    </div>
    ...
    <script>
        ...
        function buildLayout() {{
            ...
            if (isConteos) {{
                pageTitleText = "Página 1: Personas con discapacidad o dificultades";
                pageSubtitleText = "Comparación de magnitudes absolutas entre Región Metropolitana, comuna y manzana.";
            }}

            if (isPorcentajes) {{
                pageTitleText = "Página 2: Discapacidad y dificultades sobre población total";
                pageSubtitleText = "Comparación proporcional sobre la población total de cada nivel territorial.";
            }}

            if (isBrechas) {{
                pageTitleText = "Página 3: Matriz de color de brechas";
