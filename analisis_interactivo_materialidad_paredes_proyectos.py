...
# =========================================================
# VARIABLES REALES DE MATERIALIDAD DE PAREDES
# =========================================================
# Denominador: n_vp_ocupada = viviendas particulares ocupadas
#
# Columnas reales detectadas en tu base:
...
VARIABLES_PAREDES = {
    "n_mat_paredes_albanileria": "Albañilería",
    "n_mat_paredes_hormigon": "Hormigón armado",
    "n_mat_paredes_tabique_forrado": "Tabique forrado por ambas caras",
    "n_mat_paredes_tabique_sin_forro": "Tabique sin forro interior",
    "n_mat_paredes_artesanal": "Adobe, barro, quincha, pirca u otro artesanal",
    "n_mat_paredes_precarios": "Materiales precarios o de desecho"
}

VARIABLES_NECESARIAS = list(VARIABLES_PAREDES.keys()) + ["n_vp_ocupada"]
...
for col in VARIABLES_NECESARIAS:
    if col not in df.columns:
        print("\nColumnas de paredes encontradas en la base:")
        for c in df.columns:
            if (
                "pared" in c.lower()
                or "hormigon" in c.lower()
                or "tabique" in c.lower()
                or "alban" in c.lower()
                or "artesanal" in c.lower()
                or "precario" in c.lower()
            ):
                print(" -", c)
...
print("=" * 90)
print("ANÁLISIS INTERACTIVO MATERIALIDAD DE PAREDES - PROYECTOS")
print("=" * 90)
print("\nBase completa:", df.shape)
print("Región Metropolitana:", region_rm.shape)

print("\nColumnas utilizadas:")
for col, label in VARIABLES_PAREDES.items():
    print(f"{label}: {col}")
print("Denominador: n_vp_ocupada")
...
orden_variables = list(VARIABLES_PAREDES.keys())
orden_labels = list(VARIABLES_PAREDES.values())
...
for nombre_proyecto, manzent in PROYECTOS.items():
    ...
    for nivel_key, base in bases.items():
        ...
        total_viviendas = float(suma["n_vp_ocupada"])

        valores_conteos = [float(suma[var]) for var in orden_variables]
        valores_porcentajes = [
            safe_pct(float(suma[var]), total_viviendas)
        ...
        for var, label, valor_conteo, valor_pct in zip(
            ...
            registros_excel.append({
                ...
                "MANZENT": manzent,
                "Variable": var,
                "Materialidad de paredes": label,
                "Nivel": nivel_key,
                "Viviendas": float(valor_conteo),
                "Viviendas particulares ocupadas": float(total_viviendas),
                "% sobre viviendas particulares ocupadas": float(valor_pct)
            })
    ...
    for i, categoria in enumerate(orden_labels):
        ...
        registros_brechas_excel.append({
            ...
            "Comuna proyecto": comuna_nombre,
            "MANZENT": manzent,
            "Materialidad de paredes": categoria,
...
df_columnas_usadas.loc[len(df_columnas_usadas)] = {
    "Indicador": "Denominador: viviendas particulares ocupadas",
    "Columna base": "n_vp_ocupada"
}
...
ruta_excel = SALIDAS_DIR / "32_resumen_materialidad_paredes_proyectos.xlsx"
...
with pd.ExcelWriter(ruta_excel, engine="openpyxl") as writer:
    ...
    df_excel.to_excel(writer, sheet_name="Datos_paredes", index=False)
    df_brechas_excel.to_excel(writer, sheet_name="Brechas_paredes", index=False)
...
ruta_html = SALIDAS_DIR / "33_interactivo_materialidad_paredes_proyectos.html"
...
<head>
    <meta charset="UTF-8">
    <title>Materialidad de paredes en viviendas particulares ocupadas</title>
...
<body>
    <div class="container">
        <main class="main">
            <div class="header">
                <h1>Materialidad de paredes en viviendas particulares ocupadas</h1>
                <p id="subtitle"></p>
            </div>
        ...
        <aside class="sidebar">
            <div class="sidebar-card">
                ...
                <div class="control-block">
                    ...
                    <div class="tab-buttons">
                        <button class="tab-button active" id="btnConteos">
                            Página 1 — Viviendas por materialidad
                        </button>
                        <button class="tab-button" id="btnPorcentajes">
                            Página 2 — % sobre viviendas ocupadas
                        </button>
                ...
                <div class="note">
                    La página 2 muestra la distribución porcentual de la materialidad predominante de paredes sobre el total de viviendas particulares ocupadas de cada nivel territorial.
                </div>
            </div>
        </aside>
    </div>
    ...
    <script>
        ...
        function buildLayout() {{
            ...
            if (isConteos) {{
                pageTitleText = "Página 1: Viviendas por materialidad predominante de paredes";
                pageSubtitleText = "Comparación de magnitudes absolutas entre Región Metropolitana, comuna y manzana.";
            }}

            if (isPorcentajes) {{
                pageTitleText = "Página 2: Distribución porcentual de la materialidad de paredes";
                pageSubtitleText = "Comparación proporcional sobre viviendas particulares ocupadas.";
            }}

            if (isBrechas) {{
                pageTitleText = "Página 3: Matriz de color de brechas por materialidad de paredes";
            ...
            const xAxisTitle = isConteos
                ? "Número de viviendas"
                : "% sobre viviendas particulares ocupadas";
