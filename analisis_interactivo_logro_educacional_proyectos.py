...
# =========================================================
# VARIABLES REALES LOGRO EDUCACIONAL CINE
# =========================================================
...
#
# Denominador:
# n_per = población total.
#
# Nota:
# "Menor a primaria" se asocia a n_cine_nunca_curso_primera_infancia.
# "Avanzado" se asocia a n_cine_terciaria_maestria_doctorado.

VARIABLES_LOGRO = {
    "n_cine_primaria": "Primaria",
    "n_cine_secundaria": "Secundaria",
    "n_cine_nunca_curso_primera_infancia": "Menor a primaria",
    "n_cine_especial_diferencial": "Educación especial o diferencial",
    "n_cine_terciaria_maestria_doctorado": "Avanzado"
}

VARIABLES_NECESARIAS = list(VARIABLES_LOGRO.keys()) + ["n_per"]
...
for col in VARIABLES_NECESARIAS:
    if col not in df.columns:
        print("\nColumnas CINE encontradas en la base:")
        for c in df.columns:
            if "cine" in c.lower() or "asistencia" in c.lower():
                print(" -", c)
...
print("=" * 90)
print("ANÁLISIS INTERACTIVO LOGRO EDUCACIONAL CINE - PROYECTOS")
print("=" * 90)
print("\nBase completa:", df.shape)
print("Región Metropolitana:", region_rm.shape)

print("\nColumnas utilizadas:")
for col, label in VARIABLES_LOGRO.items():
    print(f"{label}: {col}")
print("Denominador: n_per")
...
orden_variables = list(VARIABLES_LOGRO.keys())
orden_labels = list(VARIABLES_LOGRO.values())
...
for nombre_proyecto, manzent in PROYECTOS.items():
    ...
    for nivel_key, base in bases.items():
        ...
        total_personas = float(suma["n_per"])

        valores_conteos = [float(suma[var]) for var in orden_variables]

        valores_porcentajes = [
            safe_pct(valor, total_personas)
        ...
        for var, label, valor_conteo, valor_pct in zip(
            ...
            registros_excel.append({
                ...
                "MANZENT": manzent,
                "Variable": var,
                "Logro educacional CINE": label,
                "Nivel": nivel_key,
                "Personas": float(valor_conteo),
                "Población total": float(total_personas),
                "% sobre total de personas": float(valor_pct)
            })
    ...
    for i, categoria in enumerate(orden_labels):
        ...
        registros_brechas_excel.append({
            ...
            "Comuna proyecto": comuna_nombre,
            "MANZENT": manzent,
            "Logro educacional CINE": categoria,
...
df_columnas_usadas.loc[len(df_columnas_usadas)] = {
    "Indicador": "Denominador: población total",
    "Columna base": "n_per"
}
...
ruta_excel = SALIDAS_DIR / "36_resumen_logro_educacional_proyectos.xlsx"
...
with pd.ExcelWriter(ruta_excel, engine="openpyxl") as writer:
    ...
    df_excel.to_excel(writer, sheet_name="Datos_logro_CINE", index=False)
    df_brechas_excel.to_excel(writer, sheet_name="Brechas_logro_CINE", index=False)
...
ruta_html = SALIDAS_DIR / "37_interactivo_logro_educacional_proyectos.html"
...
<head>
    <meta charset="UTF-8">
    <title>Logro educacional más alto alcanzado CINE</title>
...
<body>
    <div class="container">
        <main class="main">
            <div class="header">
                <h1>Logro educacional más alto alcanzado (CINE)</h1>
                <p id="subtitle"></p>
            </div>
        ...
        <aside class="sidebar">
            <div class="sidebar-card">
                ...
                <div class="control-block">
                    ...
                    <div class="tab-buttons">
                        <button class="tab-button active" id="btnConteos">
                            Página 1 — Personas por nivel educacional
                        </button>
                        <button class="tab-button" id="btnPorcentajes">
                            Página 2 — % sobre total de personas
                        </button>
                ...
                <div class="note">
                    La página 2 muestra la distribución porcentual del logro educacional más alto alcanzado sobre el total de personas de cada nivel territorial.
                </div>
            </div>
        </aside>
    </div>
    ...
    <script>
        ...
        function buildLayout() {{
            ...
            if (isConteos) {{
                pageTitleText = "Página 1: Personas por logro educacional";
                pageSubtitleText = "Comparación de magnitudes absolutas entre Región Metropolitana, comuna y manzana.";
            }}

            if (isPorcentajes) {{
                pageTitleText = "Página 2: Distribución porcentual del logro educacional";
                pageSubtitleText = "Comparación proporcional sobre el total de personas.";
            }}

            if (isBrechas) {{
                pageTitleText = "Página 3: Matriz de color de brechas por logro educacional";
