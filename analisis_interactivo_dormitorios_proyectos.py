...
VARIABLES_DORMITORIOS = {
    "n_dormitorios_1": "1 dormitorio",
    "n_dormitorios_2": "2 dormitorios",
    "n_dormitorios_3": "3 dormitorios",
    "n_dormitorios_4": "4 dormitorios",
    "n_dormitorios_5": "5 dormitorios",
    "n_dormitorios_6_o_mas": "6 o más dormitorios"
}

VARIABLES_NECESARIAS = list(VARIABLES_DORMITORIOS.keys())
...
print("=" * 90)
print("ANÁLISIS INTERACTIVO DORMITORIOS - PROYECTOS")
print("=" * 90)
...
orden_variables = list(VARIABLES_DORMITORIOS.keys())
orden_labels = list(VARIABLES_DORMITORIOS.values())
...
for nombre_proyecto, manzent in PROYECTOS.items():
    ...
    for nivel_key, base in bases.items():
        ...
        total_dormitorios_informados = float(suma.sum())
        totales_por_nivel[nivel_key] = total_dormitorios_informados

        valores_conteos = [float(suma[var]) for var in orden_variables]
        valores_porcentajes = [
            safe_pct(float(suma[var]), total_dormitorios_informados)
        ...
        for var, label, valor_conteo, valor_pct in zip(
            ...
            registros_excel.append({
                ...
                "MANZENT": manzent,
                "Variable": var,
                "Cantidad de dormitorios": label,
                "Nivel": nivel_key,
                "Viviendas": float(valor_conteo),
                "Total viviendas con dormitorio informado": float(total_dormitorios_informados),
                "% sobre viviendas con dormitorio informado": float(valor_pct)
            })
    ...
    for i, categoria in enumerate(orden_labels):
        ...
        registros_brechas_excel.append({
            ...
            "Comuna proyecto": comuna_nombre,
            "MANZENT": manzent,
            "Cantidad de dormitorios": categoria,
...
ruta_excel = SALIDAS_DIR / "20_resumen_dormitorios_proyectos.xlsx"

with pd.ExcelWriter(ruta_excel, engine="openpyxl") as writer:
    df_detalle.to_excel(writer, sheet_name="Detalle_proyectos", index=False)
    df_excel.to_excel(writer, sheet_name="Datos_dormitorios", index=False)
    df_brechas_excel.to_excel(writer, sheet_name="Brechas_dormitorios", index=False)
...
ruta_html = SALIDAS_DIR / "21_interactivo_dormitorios_proyectos.html"
...
<head>
    <meta charset="UTF-8">
    <title>Comparación territorial de viviendas según cantidad de dormitorios</title>
...
<body>
    <div class="container">
        <main class="main">
            <div class="header">
                <h1>Comparación territorial de viviendas según cantidad de dormitorios</h1>
                <p id="subtitle"></p>
            </div>
        ...
        <aside class="sidebar">
            <div class="sidebar-card">
                ...
                <div class="control-block">
                    ...
                    <div class="tab-buttons">
                        <button class="tab-button active" id="btnConteos">
                            Página 1 — Viviendas por dormitorios
                        </button>
                        <button class="tab-button" id="btnPorcentajes">
                            Página 2 — Distribución porcentual
                        </button>
                ...
                <div class="note">
                    La página 2 muestra la distribución porcentual sobre viviendas con dormitorio informado.
                    La página 3 muestra brechas en puntos porcentuales respecto de la Región Metropolitana y la comuna.
                </div>
            </div>
        </aside>
    </div>
    ...
    <script>
        ...
        function hoverTemplate(levelKey, page) {{
            ...
            if (levelKey === "regional") {{
                return (
                    ...
                    "<b>Cantidad de dormitorios:</b> %{{x}}<br>" +
        ...
        function buildBrechaTrace() {{
            ...
            return [{{
                ...
                hovertemplate:
                    ...
                    "<b>Cantidad de dormitorios:</b> %{{y}}<br>" +
        ...
        function buildLayout() {{
            ...
            if (isConteos) {{
                pageTitleText = "Página 1: Viviendas según cantidad de dormitorios";
                pageSubtitleText = "Comparación de magnitudes absolutas entre Región Metropolitana, comuna y manzana.";
            }}

            if (isPorcentajes) {{
                pageTitleText = "Página 2: Distribución porcentual según cantidad de dormitorios";
                pageSubtitleText = "Comparación proporcional sobre viviendas con dormitorio informado.";
            }}

            if (isBrechas) {{
                pageTitleText = "Página 3: Matriz de color de brechas por cantidad de dormitorios";
            ...
            const yAxisTitle = isConteos
                ? "Número de viviendas"
                : "% sobre viviendas con dormitorio informado";
